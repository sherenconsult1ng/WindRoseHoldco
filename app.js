...
function openFilePreview(url, filename) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      // If data: or blob: -> do nothing here; let the calling anchor's native
      // href/target/download behaviour handle it. Scripted window.open()/
      // location.href navigation to a data: URL is blocked as an unsafe
      // top-level navigation by Chrome, Edge and Firefox, even though a plain
      // user click (or right-click "open in new tab") on the same href works
      // fine. Re-triggering it via JS is what breaks click-to-view/download.
